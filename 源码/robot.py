...
class Robot(object):
    ...
    def MaxValue_po(self, player, enemy, hardlevel):
        """算出最大价值的点"""
        ...
        _point = []
        # for p in points:
        #     value_list.append(p[2])
        # arr = list()
            # (填充操作省略）
        points.sort(key=lambda list: list[2])
        max_k1 = points[-1]
        i = len(points) - 1
        if hardlevel.get() == 0:
            _point = points[-1]
            ...
            if a % 2 != 0:
                max_k2 = points[len(points)-1]
            else:
                while True:
                    i-=1
                    if max_k1[2] != points[i][2]:
                        max_k2 = points[i]
                        break
            while True:
                i-=1
                if max_k2[2] != points[i][2]:
                    max_k3 = points[i]
                    break
            print(hardlevel.get())
            if hardlevel.get() == 1:
                _point = max_k3
            elif hardlevel.get() == 2:
                _point = max_k2
            elif hardlevel.get() == 3:
                _point = max_k1

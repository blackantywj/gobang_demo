...
class GoBang(object):
    ...
    def __init__(self):
        ...
        self.window.geometry("630x470+80+80")
    ...
    def check_win(self):
        ...
        if self.have_five(self.whi_chessed) == True:
            ...
            label.place(relx=0, rely=0, x=480, y=440)
            return True
    ...
    def AIrobotChess(self):
        ...
        if self.player % 2 == 0:
            ...
            if len(self.bla_chessed) == 0 and len(self.whi_chessed) == 0:
                ...
                #机器人计算出全局价值最大的点
                # print(self.var1)
                _x, _y, _ = self.robot.MaxValue_po(1, 0, self.var1)
    ...
    def robotChess(self):
        ...
        if self.player == 0:
            ...
            if len(self.bla_chessed) == 0 and len(self.whi_chessed) == 0:
                ...
                _x, _y, _ = self.robot.MaxValue_po(0, 1, self.var1)
                # print(self.var1.get())
                newPoint = pos_in_board(_x, _y)
    ...
    def start(self):
        ...
        b3.place(relx=0, rely=0, x=495, y=30)

        b1 = Button(self.window, text="重置", command=self.resetButton)
        b1.place(relx=0, rely=0, x=495, y=60)

        b2 = Button(self.window, text="悔棋", command=self.BakcAChess)
        b2.place(relx=0, rely=0, x=495, y=90)

        b4 = Radiobutton(self.window, text="电脑执黑棋", variable=self.var, value=0, command=self.selectColor)
        b4.place(relx=0, rely=0, x=495, y=180)

        b5 = Radiobutton(self.window, text="电脑执白棋", variable=self.var, value=1, command=self.selectColor)
        b5.place(relx=0, rely=0, x=495, y=210)

        b6 = Button(self.window, text="打开棋谱", command=self.OpenFile)
        b6.place(relx=0, rely=0, x=495, y=120)

        b7 = Button(self.window, text="保存棋谱", command=self.SaveFile)
        b7.place(relx=0, rely=0, x=495, y=150)

        b8 = Radiobutton(self.window, text="神经网络", variable=self.var1, value=0, command=self.selectMathod)
        b8.place(relx=0, rely=0, x=495, y=240)

        b9 = Radiobutton(self.window, text="普通", variable=self.var1, value=1, command=self.selectMathod)
        b9.place(relx=0, rely=0, x=495, y=270)

        b10 = Radiobutton(self.window, text="中等", variable=self.var1, value=2, command=self.selectMathod)
        b10.place(relx=0, rely=0, x=495, y=300)

        b11 = Radiobutton(self.window, text="困难", variable=self.var1, value=3, command=self.selectMathod)
        b11.place(relx=0, rely=0, x=495, y=330)
        label = Label(self.window, text="落子过程", background="#F0F0F0", font=("宋体", 15, "bold"))
        label.place(relx=0, rely=0, x=495, y=360)
        self.text = Text(width=13, height=2)
        self.text.place(x=495, y=390, anchor='nw')
...
if __name__ == '__main__':
    ...
    dialog = develop()
    # game.start()
    begin.AI.clicked.connect(lambda: show_select(begin, game))
